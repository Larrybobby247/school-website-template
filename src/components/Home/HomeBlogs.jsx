  ...
  return (
    <div>
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-3'>
        {Blogs.map(blog => {
            return(
                <Link to={`/blogs/${blog.name}`} key={blog.id} className='flex flex-col gap-3 p-4 bg-white transition duration-300 shadow-md hover:shadow-xl hover:translate-y-[-4px] rounded-sm'>
